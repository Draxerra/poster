...
export default function PostListItem({ post }: PostProps) {
  ...
  return (
    <li className='flex flex-wrap items-center justify-between gap-4 p-4'>
      <Link className='underline text-blue-800' href={`/post/${optimisticPost.id}`}>{optimisticPost.post}</Link>
      <form
        action={async() => {
        ...
        ref={formRef}
      >
        <button className='group inline-flex items-center space-x-1' type='submit'>
